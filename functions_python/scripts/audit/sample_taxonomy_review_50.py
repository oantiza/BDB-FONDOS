...
def _is_reasonable_fi(doc: Dict[str, Any]) -> bool:
    ...
    if fi_duration in {"SHORT", "MEDIUM", "FLEXIBLE"}:
        return True
...
def _is_reasonable_equity(doc: Dict[str, Any]) -> bool:
    class_v2 = doc.get("classification_v2") or {}
    asset_type = normalize_text(class_v2.get("asset_type"))
    subtype = normalize_text(class_v2.get("asset_subtype"))
    style_box = normalize_text(class_v2.get("equity_style_box"))
    mcap = normalize_text(class_v2.get("market_cap_bias"))
    is_sector_fund = boolish(class_v2.get("is_sector_fund"))
    is_thematic = boolish(class_v2.get("is_thematic"))
    legacy = _legacy_blob(doc)
    ...
    if is_sector_fund or is_thematic:
        return True
...
def _is_reasonable_alternative(doc: Dict[str, Any]) -> bool:
    class_v2 = doc.get("classification_v2") or {}
    asset_type = normalize_text(class_v2.get("asset_type"))
    alternative_bucket = normalize_text(class_v2.get("alternative_bucket"))
    legacy = _legacy_blob(doc)

    if asset_type != "ALTERNATIVE":
        return False

    if alternative_bucket not in {"", "UNKNOWN"}:
        return True
...
def _is_reasonable_non_core_other(doc: Dict[str, Any]) -> bool:
    class_v2 = doc.get("classification_v2") or {}
    asset_type = normalize_text(class_v2.get("asset_type"))
    legacy = _legacy_blob(doc)

    if asset_type == "MONETARY":
        return True
    ...
    if asset_type == "ALTERNATIVE" and _is_reasonable_alternative(doc):
        return True
...
def _has_real_conflict(doc: Dict[str, Any], flags: List[str]) -> bool:
    ...
    if "UNKNOWN_SUBTYPE" in flags:
        if asset_type == "FIXED_INCOME":
            return not _is_reasonable_fi(doc)

        if asset_type == "EQUITY":
            return not _is_reasonable_equity(doc)

        if asset_type in {"MONETARY", "COMMODITIES", "REAL_ESTATE"}:
            return False

        if asset_type == "ALTERNATIVE":
            return not _is_reasonable_alternative(doc)
    ...
    if asset_type == "EQUITY":
        eq_soft_only = set(flags).issubset(
            {
        ...
        if eq_soft_only and _is_reasonable_equity(doc):
            return False

    if asset_type in {"MONETARY", "COMMODITIES", "REAL_ESTATE"}:
        soft_only = set(flags).issubset(
            {
                "UNKNOWN_SUBTYPE",
                "MISSING_REGION_PRIMARY",
                "LOW_CONFIDENCE",
        ...
        if soft_only and _is_reasonable_non_core_other(doc):
            return False
    ...
    if asset_type == "ALTERNATIVE":
        ...
        if soft_only and _is_reasonable_alternative(doc):
            return False
...
def compute_audit_flags(doc: Dict[str, Any]) -> List[str]:
    ...
    if not region_primary or region_primary == "UNKNOWN":
        if not (
            (asset_type == "FIXED_INCOME" and _is_reasonable_fi(doc))
            or (asset_type == "EQUITY" and _is_reasonable_equity(doc))
            or (asset_type in {"MONETARY", "COMMODITIES", "REAL_ESTATE"})
            or (asset_type == "ALTERNATIVE" and _is_reasonable_alternative(doc))
        ):

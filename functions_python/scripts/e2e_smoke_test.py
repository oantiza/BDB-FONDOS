...
def print_result(name, res):
    ...
    if "metrics" in res:
        m = res["metrics"]
        print(f"Metrics: Return: {m.get('expected_return', m.get('return', 0)):.4%}, Vol: {m.get('volatility', 0):.4%}, Sharpe: {m.get('sharpe_ratio', m.get('sharpe', 0)):.4f}")
    
    if "explainability" in res:
        print(f"Explainability applied constraints: {res['explainability'].get('applied_constraints', {})}")
    print(f"{'='*50}\\n")

def test_normal_optimization():
    print("Running: Normal Optimization")
    assets_list = [
        "LU0293313671", "LU0117858752", "ES0182105033", 
        "ES0161992005", "ES0142167032", "ES0138936036", 
        "LU0835722488", "LU1066281574"
    ]
    risk_level = 5
...
def test_analyzer(opt_res):
    ...
    if not opt_res or "weights" not in opt_res:
        print("❌ Cannot run analyzer: Previous optimization failed or returned no weights.")
        return False, None

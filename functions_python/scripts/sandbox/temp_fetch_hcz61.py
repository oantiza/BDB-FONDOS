...
STATUS: ACTIVE
CATEGORY: sandbox
PURPOSE: Scraper temporal para fondo específico HCZ61.
SAFE_MODE: SANDBOX
RUN: python -m scripts.sandbox.temp_fetch_hcz61
"""
import os
import sys
...
if __name__ == "__main__":
    ...
    if db:
        isin = "IE00B03HCZ61"
        print(f"Buscando histÃ³rico para {isin} desde 2015...")

        # 1. Fetch history from EODHD overriding whatever there is
        res = update_single_fund_history(
            db, isin, mode="overwrite", from_date="2015-01-01"
        )
        print(f"Result EODHD Fetch: {res}")

        # 2. Recalculate metrics (std_perf) with the newly fetched data
        if res and res.get("success"):
            print(f"Refrescando mÃ©tricas std_perf para {isin}...")

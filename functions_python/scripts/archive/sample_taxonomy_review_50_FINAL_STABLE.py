...
def _has_real_conflict(doc: Dict[str, Any], flags: List[str]) -> bool:
    ...
    if "UNKNOWN_SUBTYPE" in flags:
        # FI: sigue siendo relevante salvo cuando ya está razonablemente resuelto
        if asset_type == "FIXED_INCOME":
            if _is_reasonable_fi(doc):
                return False
            return True

        # Equity: si el equity ya es razonable, no lo tratamos como conflicto duro
        if asset_type == "EQUITY":
            if _is_style_missing_but_not_conflict(doc):
                return False
            return True

        # Monetary: subtype desconocido no es problema real
        if asset_type == "MONETARY":
            return False

        # Commodities: subtype desconocido no es problema real
        if asset_type == "COMMODITIES":
            return False

        # Real estate: subtype desconocido no es problema real
        if asset_type == "REAL_ESTATE":
            return False

        # Alternative: si ya tiene bucket o el legacy lo deja claro, no es conflicto duro
    ...
    # Soft cases para clases no problemáticas
    ...
    if asset_type == "ALTERNATIVE":
        ...
        if soft_only and (
            alternative_bucket not in {"", "UNKNOWN"}
            or contains_any(
                legacy,
                [
                    "ALT -",

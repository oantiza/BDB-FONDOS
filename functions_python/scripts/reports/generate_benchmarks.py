...
STATUS: ACTIVE
CATEGORY: reports
PURPOSE: Genera benchmarks de mercado para comparación de carteras.
SAFE_MODE: READ_ONLY
...
def calculate_metrics(nav_series, rf_rate):
    ...
    if not nav_3y.empty and len(nav_3y) > 50:
        ...
        if excess_returns_3y.std() > 0:
            sharpe_3y = (excess_returns_3y.mean() / excess_returns_3y.std()) * np.sqrt(
                252
            )
        else:
            sharpe_3y = 0.0
...
def main():
    ...
    print(f"\nð Dynamic Risk-Free Rate loaded from ECB: {rf_rate * 100:.3f}%")
    ...
    print(
        f"Longest common timeline: {df_merged.index[0].date()} to {df_merged.index[-1].date()} ({len(df_merged)} points)"
    )
    ...
    for name, weights in PROFILES.items():
        ...
        doc_data = {
            "name": name,
            "weights": {ISIN_A: w_a, ISIN_B: w_b, "CASH": w_c},
            "std_perf": metrics,
        ...
        print(f"â Prepared {name} for Firestore.")
    ...
    for name in ["CONSERVADOR", "MODERADO", "EQUILIBRADO", "DINAMICO", "AGRESIVO"]:
        m = results[name]
        print(
            f"| {name} | {m['return'] * 100:.2f}% | {m['volatility'] * 100:.2f}% | {m['sharpe']:.2f} | {m['max_drawdown'] * 100:.2f}% |"
        )

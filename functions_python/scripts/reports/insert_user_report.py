...
STATUS: ACTIVE
CATEGORY: reports
PURPOSE: Inserta un reporte específico de usuario en la base de datos.
...
def insert_report():
    ...
    print("ð Inserting MONTHLY_PORTFOLIO report to bdb-fondos...")

    try:
        # Add to Firestore
        doc_ref = db.collection("analysis_results").add(monthly_report)
        print(f"â Successfully added report with ID: {doc_ref[1].id}")
        ...
        print(f"â Error inserting report: {e}")
        return False
...
if __name__ == "__main__":
    ...
    if success:
        print("\nð Report successfully migrated to bdb-fondos!")

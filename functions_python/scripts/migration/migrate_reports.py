...
STATUS: ACTIVE
CATEGORY: migration
PURPOSE: Migración estructural de reportes de versiones anteriores.
...
def migrate_reports():
    ...
    """
    print("ð Starting migration from boutique-financiera-app to bdb-fondos...")
    ...
    for report_doc in reports:
        ...
        print(
            f"â Migrated: {report_data.get('type', 'Unknown')} - {report_data.get('date', 'No date')}"
        )
        migrated_count += 1

    print(f"\nð Migration complete! Migrated {migrated_count} reports.")
...
if __name__ == "__main__":
    try:
        ...
        print(f"\nâ¨ Successfully migrated {count} reports to bdb-fondos")
    except Exception as e:
        print(f"\nâ Error during migration: {e}")
        print(

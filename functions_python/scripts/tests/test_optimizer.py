...
STATUS: ACTIVE
CATEGORY: tests
PURPOSE: Pruebas técnicas del motor de optimización.
...
def test():
    print(
        "Testing run_optimization with EXACT rebalance constraints (disable_profile_rules + max_sharpe)..."
    )
    ...
    try:
        ...
        print(
            f"â Success! solver_path={res.get('solver_path')}, status={res.get('status')}"
        )

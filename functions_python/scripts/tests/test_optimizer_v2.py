...
def test_optimization_v2():
    ...
    ef_constrained = FinancialEngine.optimize_efficient_frontier(mu, S, constraints=constraints, exposures_v2=exposures_v2)
    ...
    tech_exposure = sum(cleaned_weights[t] * (exposures_v2[t].sectors.get("TECHNOLOGY", 0.0) / 100.0) for t in tickers)

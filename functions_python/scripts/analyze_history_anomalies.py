...
def analyze_anomalies(threshold=0.20):
    print(
        f"🔍 Analyzing history for anomalies (threshold > {threshold * 100}% daily change)..."
    )
    ...
    for doc in docs:
        ...
        if not anomalies.empty:
            ...
            anomalous_funds.append(
                {
                    ...
                    "sample_jumps": anomalies[["date", "nav_prev", "nav", "pct_change"]]
                    .head(3)
                    .to_dict("records"),
                }
            )
            ...
            print(
                f"⚠️ {doc.id} - Anomaly! Max jump: {max_jump * 100:.2f}%, Max drop: {min_jump * 100:.2f}%. Jumps: {len(anomalies)}"
            )
    ...
    if anomalous_funds:
        ...
        top_jumps = sorted(
            anomalous_funds, key=lambda x: x["max_jump_pct"], reverse=True
        )[:10]
        for f in top_jumps:
        ...
        for f in anomalous_funds[:3]:
            ...
            for s in f["sample_jumps"]:
                print(
                    f"  {s['date'].strftime('%Y-%m-%d')}: {s['nav_prev']} -> {s['nav']} ({s['pct_change'] * 100:.2f}%)"
                )

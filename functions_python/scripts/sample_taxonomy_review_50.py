...
def _deduce_equity_subtype(ms_cat_up: str, name_up: str, klass: ClassificationV2) -> AssetSubtypeV2:
    ...
    if "EUROPE" in text or any(x in text for x in ["IBERIA", "SPAIN", "ESPAÑA", "ITALY", "GERMANY", "FRANCE", "SWITZERLAND"]):
...
def _deduce_fixed_income_subtype(ms_cat_up: str, name_up: str) -> AssetSubtypeV2:
    ...
    if any(x in text for x in ["CORPORATE", "CREDIT", "FLOATING RATE", "SHORT DURATION", "SHORT TERM", "0-2", "0-5", "AHORRO", "PAGARES", "PAGARÉS", "HORIZON", "HORIZONTE", "SECURITY", "SECURITE", "TESORERIA", "TREASORERIE"]):
    ...
    if klass.asset_type == AssetClassV2.EQUITY:
        ...
        if not is_risky and any(x in text for x in strong_defensive):
            ...
            if klass.fi_duration_bucket == FIDurationBucketV2.UNKNOWN:
                if any(x in text for x in ["SHORT", "CORTO", "SHORT TERM", "SHORT DURATION", "0-2", "0-5", "AHORRO", "PAGARES", "PAGARÉS"]):

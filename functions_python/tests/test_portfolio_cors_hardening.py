"""
Test: CORS hardening for endpoints_portfolio.py
Validates that cors_config uses an explicit allowlist, not wildcard '*'.
"""
import unittest
import inspect

from api.endpoints_portfolio import cors_config


class TestPortfolioCorsConfig(unittest.TestCase):
    """Verify endpoints_portfolio.py cors_config is hardened."""
...
class TestPortfolioCorsSourceInvariant(unittest.TestCase):
    """Source-level invariant: no wildcard CORS in endpoints_portfolio.py."""

    def test_no_wildcard_cors_in_source(self):
        """The source code of endpoints_portfolio must not contain cors_origins='*'."""
        import api.endpoints_portfolio as mod

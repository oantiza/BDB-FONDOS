...
def dummy_price_data():
    ...
    df_a = pd.Series(
        np.cumprod(1 + np.random.normal(0.0005, 0.015, 500)) * 100, index=dates
    )
    # Low vol series
    df_b = pd.Series(
        np.cumprod(1 + np.random.normal(0.0001, 0.003, 500)) * 100, index=dates
    )
...
def test_optimizer_fallback_path(mock_data_fetcher_class, dummy_db, dummy_price_data):
    ...
    bad_data = {
        "FundA": pd.Series(
            [1.0] * 500, index=pd.date_range("2020-01-01", periods=500, freq="B")
        )
    }
    ...
    assert res["status"] in [
        "optimal",
        "fallback",
    ]  # If fallback min_vol succeeds it sets ok

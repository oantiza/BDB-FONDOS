...
class PrivateBankingReport(FPDF):
    ...
    def footer(self):
        ...
        self.cell(
            0,
            10,
            f"Página {self.page_no()} | Confidencial - Generado por Gemini 3 Pro",
            0,
            0,
            "C",
        )
...
def generate_pdf_from_data(report_data):
    ...
    """
    pdf = PrivateBankingReport(
        report_data.get("title", "Informe de Estrategia"), report_data.get("date", "")
    )
    pdf.add_page()
    ...
    pdf.multi_cell(
        0, 10, report_data.get("title", "Informe Estratégico Global"), align="C"
    )
    pdf.ln(5)
    ...
    pdf.cell(
        0,
        5,
        f"FECHA: {report_data.get('date', '')} | RÉGIMEN: {report_data.get('regime', 'N/A')}",
        0,
        1,
        "C",
    )
    ...
    alloc = report_data.get("asset_allocation", []) or report_data.get(
        "model_portfolio", []
    )
    ...
    col_widths = (
        [40, 30, 30] if chart_stream else [50, 40, 90]
    )  # Compact if chart exists
    headers = (
        ["ACTIVO", "VISIÓN", "RACIONAL (Resumido)"]
        if chart_stream
        else ["ACTIVO", "VISIÓN", "RACIONAL"]
    )
    ...
    for item in alloc:
        ...
        if view in ["SOBREPONDERAR", "OVERWEIGHT"]:
            pdf.set_text_color(0, 100, 0)
        elif view in ["INFRAPONDERAR", "UNDERWEIGHT"]:
        ...
        view_label = view.replace("SOBREPONDERAR", "SOBRE").replace(
            "INFRAPONDERAR", "INFRA"
        )

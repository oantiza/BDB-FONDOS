...
def process_fund(db, isin, dry_run=False):
    ...
    if not dry_run and (anomalies_removed > 0 or has_gaps or has_outliers):
        ...
        if anomalies_removed > 0:
            ...
            if is_dict_format:
                new_history = {
                    str(k.date()): float(v) for k, v in clean_df["price"].items()
                }
            else:
                new_history = []
                for item in history:
        ...
        if has_gaps or has_outliers:
            updates = {}
            if has_outliers:
                updates["data_quality.has_outliers"] = True
            if has_gaps:
                updates["data_quality.has_gaps"] = True
...
def run_nav_validation_pipeline(db, dry_run=False):
    logger.info(
        f"🚀 Starting Automated NAV Validation Pipeline (Dry Run: {dry_run})..."
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = {
            executor.submit(process_fund, db, doc.id, dry_run): doc for doc in funds
        }

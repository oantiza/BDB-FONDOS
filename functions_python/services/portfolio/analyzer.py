...
from services.portfolio.utils import _classify_asset, _to_float
...
def analyze_portfolio(portfolio_weights: dict, db) -> dict:
    ...
    actual_start_date = first_valid_indices.max() if not first_valid_indices.empty else ideal_start_date
    ...
    valid_weights = {isin: w for isin, w in portfolio_weights.items() if isin in universe}
    ...
    for col in corr_matrix.columns:
        corr_json[col] = {row: float(corr_matrix.loc[row, col]) for row in corr_matrix.index}
    ...
    opinion = []
    if port_sharpe < 0.5:
        opinion.append("El ratio de Sharpe actual de la cartera es bajo. Sugiere que el retorno ajustado por riesgo podría mejorarse considerablemente.")
    elif port_sharpe > 1.0:
        opinion.append("La cartera muestra una excelente eficiencia histórica en términos de rentabilidad-riesgo.")
    else:
        opinion.append("La cartera tiene un rendimiento histórico aceptable en términos de rentabilidad-riesgo.")
        
    if high_corr_pairs:
        pairs_str = ", ".join([f"{p['asset1']} y {p['asset2']} ({p['correlation']:.2f})" for p in high_corr_pairs])
        opinion.append(f"Alerta de concentración: Hemos detectado fondos con alta correlación entre sí (>0.80): {pairs_str}. Esto significa que tienden a moverse en la misma dirección y reducen el beneficio de la diversificación. Considera sustituir uno de ellos.")
    else:
        opinion.append("La cartera presenta una buena diversificación; no se han detectado fondos con correlaciones excesivamente altas entre sí.")
    ...
    for pair in high_corr_pairs:
        ...
        try:
            alt_query = db.collection('funds_v3')\
            ...
            for doc in alt_query:
                if doc.id not in universe: # Do not suggest what they already have
                    ...
                    found_alts.append({
                        ...
                        "sharpe": _to_float(alt_data.get('std_perf', {}).get('sharpe', 0.0), 0.0),
            ...
            print(f"Error querying alternatives: {e}")

...
def is_fund_eligible_for_profile(
    asset_meta: Dict[str, Any], risk_profile: int
) -> Tuple[bool, str]:
    """
    ...
    if risk_profile <= 2:
        if not is_suitable_low_risk:
            return (
                False,
                f"Fund flagged as not suitable for low risk. Type: {asset_type}, Subtype: {asset_subtype}",
            )
        ...
        if exp_v2:
            ...
            if real_eq > 30:
                return (
                    False,
                    f"Hard exclusion: Real economic equity exposure is {real_eq}% (>30%).",
                )

    # 2. Conservative / Moderate-Low Profiles (3-4)
    if risk_profile <= 4:
        if (
            risk_bucket == RiskBucketV2.HIGH.value
            and asset_type != AssetClassV2.EQUITY.value
        ):
        ...
        if exp_v2:
            real_eq = float(
                exp_v2.get("economic_exposure", {}).get("equity", 0.0) or 0.0
            )
        ...
        if class_v2.get("is_sector_fund"):
            return (
                False,
                f"Sector funds ({class_v2.get('sector_focus')}) are excluded for profiles <= 4.",
            )
        ...
        if (
            ...
            return (
                False,
                f"Asset Subtype {asset_subtype} is excluded for profiles <= 4.",
            )
    ...
    if risk_profile <= 7:
        if exp_v2 and class_v2.get("is_sector_fund"):
            if (
                class_v2.get("sector_focus") == SectorFocusV2.HEALTHCARE.value
                and risk_profile < 6
            ):
...
def get_economic_bucket(asset_meta: Dict[str, Any]) -> str:
    ...
    if asset_type == AssetClassV2.FIXED_INCOME.value:
        if subtype in [
            AssetSubtypeV2.HIGH_YIELD_BOND.value,
            AssetSubtypeV2.EMERGING_MARKETS_BOND.value,
            AssetSubtypeV2.CONVERTIBLE_BOND.value,
        ]:

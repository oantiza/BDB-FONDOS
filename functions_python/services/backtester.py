...
def _fetch_and_process_data(assets_list, db, periods, fetcher=None):
    ...
    for col in df.columns:
        if df[col].count() < 5:  # Hardening: Enforce minimum 5 observations to allow newer assets
...
def _compute_metrics(df_master, period, weights_map, synthetic_used, fetcher):
    ...
    # Dynamic Weighting Logic: Do NOT drop missing values to allow dynamic backfilling

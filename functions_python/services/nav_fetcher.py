...
async def process_batch(db, funds_batch, session, lookback_date):
    ...
    for doc in funds_batch:
        ...
        raw_ticker = data.get("eod_ticker")
        if not raw_ticker:
            ticker_used = f"{isin}.EUFUND"
        elif "." not in raw_ticker:
            ticker_used = f"{raw_ticker}.EUFUND"
        else:
    ...
    hist_refs = [
        db.collection("historico_vl_v2").document(f[0].id) for f in funds_to_update
    ]
    ...
    for i, hist_snap in enumerate(hist_docs):
        ...
        existing_history = (
            hist_snap.to_dict().get("history", []) if hist_snap.exists else []
        )
...
def update_single_fund_history(db, isin, mode="merge", from_date=None, to_date=None):
    ...
    async def runner():
        async with aiohttp.ClientSession() as session:
            ...
            print(
                f"🔄 Updating {isin} ({ticker}) Mode={mode} From={from_date} To={to_date}"
            )

            # 2. Descargar
            ticker_res, new_data = await fetch_eodhd_data(
            ...
            if not new_data:
                return {
                    "success": False,
                    "error": "No data received from EODHD",
                    "ticker": ticker,
                }

...
def get_covariance_matrix(
    df_prices: pd.DataFrame, frequency=TRADING_DAYS_PER_YEAR
) -> pd.DataFrame:
    """
...
def get_expected_returns(
    df_prices: pd.DataFrame, frequency=TRADING_DAYS_PER_YEAR, method="ema"
) -> pd.Series:
    """
    ...
    if method == "ema":
        return expected_returns.ema_historical_return(
            df_prices, frequency=frequency, span=frequency
        )
    elif method == "mean":
...
def calculate_portfolio_metrics(
    weights_dict: dict, mu_series: pd.Series, S_df: pd.DataFrame, rf_rate: float
):
    """
...
def calculate_historical_metrics(
    df_series: pd.Series, risk_free_annual=0.0, method="geometric"
):
    """
    ...
    vol = max(
        float(vol), 0.0
    )  # Cross-Module Consistency: Ensure non-negative volatility

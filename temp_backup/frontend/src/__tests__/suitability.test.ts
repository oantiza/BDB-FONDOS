...
type AssetClass = 'RV' | 'RF' | 'Monetario' | 'Mixto' | 'Retorno Absoluto' | 'Otros';
...
function getAssetClassFromV2(v2AssetType: string | undefined, legacyDerived?: string): AssetClass {
  if (v2AssetType) {
    ...
    if (v2AssetType === 'ALTERNATIVE') return 'Retorno Absoluto';
    return 'Otros';
  }
  // Legacy fallback
...
type SidebarCategory = 'RV' | 'RF' | 'RF - Soberana' | 'RF - Corporativa' | 'RF - High Yield' |
  'Monetario' | 'Mixto' | 'Retorno Absoluto' | 'RV - Tecnología' | 'RV - Salud' | 'ALL';
...
function matchesSidebarCategory(
  ...
  if (v2) {
    ...
    if (category === 'Retorno Absoluto') return v2.asset_type === 'ALTERNATIVE';
    return false;
  }
  ...
  if (category === 'Retorno Absoluto') return fc === 'RETORNO ABSOLUTO';
  return false;
}
...
describe('getAssetClass V2 mapping', () => {
  ...
  it('maps ALTERNATIVE → Retorno Absoluto', () => {
    expect(getAssetClassFromV2('ALTERNATIVE')).toBe('Retorno Absoluto');
  });
...
describe('Sidebar filter V2 matching', () => {
  ...
  it('alternative matches Retorno Absoluto', () => {
    const v2 = { asset_type: 'ALTERNATIVE' };
    expect(matchesSidebarCategory('Retorno Absoluto', v2)).toBe(true);

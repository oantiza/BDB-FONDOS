    ...
    try:
        ...
        for doc in docs:
                ...
                for item in history:
                    if not isinstance(item, dict):
                        results["errors"].append(
                            f"{isin}: Item not dict ({type(item)})"
                        )
                        break
                    ...
                    if not date_val or val_val is None:
                        results["errors"].append(
                            f"{isin}: Missing date/value in item {item}"
                        )
                        break
                    ...
                    if (
                        ...
                        results["errors"].append(
                            f"{isin}: Invalid date format '{date_val}'"
                        )
                        break
                    ...
                    if not isinstance(val_val, (int, float)):
                        results["errors"].append(
                            f"{isin}: Invalid value type '{type(val_val)}' for '{val_val}'"
                        )
                        break
            ...
            # If it's a dict (Map)
                ...
                results["errors"].append(
                    f"{isin}: Unexpected history type {type(history)}"
                )

...
def generate_efficient_frontier(assets_list, db, portfolio_weights=None, period="3y"):
    ...
    try:
        ...
        # [White Lie] Calculate CAGR for display purposes
        cagr_series = get_expected_returns(df, method="geometric")
        ...
        if len(df.columns) >= 2:
            ...
            try:
                ...
                for v_raw, _, w_raw in zip(
                    frontier_vol, frontier_ret, frontier_weights
                ):
                    try:
                        v = float(v_raw)
                        # We use the raw weight mix on the cagr_series for display
                        r_cagr = float(np.sum(w_raw * cagr_series))
                        if np.isnan(v) or np.isnan(r_cagr) or v <= 0:
                            continue
                        frontier_points.append(
                            {"x": round(v, 4), "y": round(r_cagr, 4)}
                        )
                    except:
                        continue
                    ...
                    if max_r > min_r:
                        ...
                        for tr in target_returns:
                            try:
                                ...
                                w_arr = np.array(
                                    [weights_dict.get(c, 0) for c in df.columns]
                                )
                                r_cagr = float(np.sum(w_arr * cagr_series))
                                frontier_points.append(
                                    {"x": round(float(vol), 4), "y": round(r_cagr, 4)}
                                )
                            except:
        ...
        if portfolio_weights:
            try:
                ...
                if w_total > 0:
                    ...
                    port_ret = float(np.sum(w_arr * cagr_series))
        ...
        result = {
            ...
            "math_data": {
                ...
                "expected_returns": {
                    k: float(v) for k, v in cagr_series.to_dict().items()
                },

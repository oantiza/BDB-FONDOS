...
def _classify_asset(ticker: str, asset_metadata=None) -> str:
    ...
    if class_v2:
        ac_v2 = class_v2.get("asset_class")
...
def _allocation_vectors(tickers: list, asset_metadata=None):
    ...
    for t in tickers:
        ...
        if exp_v2:
            ...
            eq_raw = _to_float(exp_v2.get("equity", 0.0))
            bd_raw = _to_float(exp_v2.get("fixed_income", 0.0))
            cs_raw = _to_float(exp_v2.get("cash", 0.0))
            ot_raw = _to_float(exp_v2.get("other", 0.0))

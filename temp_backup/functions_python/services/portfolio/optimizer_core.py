...
def run_optimization(
    ...
    try:
        ...
        eq_vec, bd_vec, cs_vec, ot_vec, _ = _allocation_vectors(
        ...
        def _apply_standard_constraints(ef_inst, eq_v, bd_v, cs_v):
            ...
            for isin in locked_assets or []:
                if isin in universe:
                    ...
                    if lock_mode == "add_capital" and isin in fixed_weights:
                        fw_val = float(fixed_weights[isin])
                        # Prevent solver issues due to rounding or caps
                        fw_val = min(max(fw_val, 0.0), 1.0)
                        ef_inst.add_constraint(lambda w, i=idx, fw=fw_val: w[i] == fw)
                    else:
                        # Redistribute mode (or missing fixed weight): Just guarantee it's not removed
        ...
        if not solver_path or solver_path == "auto_expand_then_solve":
            try:
                ...
                elif constraints and constraints.get("objective") == "min_deviation":
            ...
            if apply_profile and risk_level_i in current_risk_buckets:
                ...
                for idx, isin in enumerate(universe):
                    ...
                    if is_eq and "RV" in bucket_cfg and bucket_cfg["RV"][1] > 0:
                        ...
                        allowed = True
                    # Fallback permissiveness for unclassified or mixed if they don't explicitly violate a 0% cap
                    # In a strict fallback, we only allow those that explicitly fit the non-zero buckets
                    if allowed:
        ...
        if s_sum > 0:
            eq_total, bd_total, cs_total, ot_total = (
                eq_total / s_sum,
                bd_total / s_sum,
                cs_total / s_sum,
                ot_total / s_sum,

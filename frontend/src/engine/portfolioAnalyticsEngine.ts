...
export async function backtestPortfolio(
  ...
  const p = (async () => {
    try {
      ...
      return normalizeResponse(unwrapResult((res as { data: unknown })?.data));
    } catch (e: unknown) {
...
export async function backtestPortfolioMulti(
  ...
  const p = (async () => {
    try {
      ...
      const res = await fn(req);
      return unwrapResult((res as { data: unknown })?.data) as MultiBacktestResponse;
    } catch (e: unknown) {
...
export async function getDashboardAnalytics(
  ...
  const periodWarnings = [
    ...
    if (res.error) return [`${label}: ${res.message || res.error}`];
    return [];
  });

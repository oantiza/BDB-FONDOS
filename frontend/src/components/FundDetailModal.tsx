      ...
      >
        ...
        >
          ...
          <div className="p-8 space-y-8 overflow-y-auto custom-scrollbar bg-white">
            {/* V2 Warnings & Legacy Fallbacks */}
            {hasLegacyTaxonomyOnly(fund) && (
              <div className="bg-amber-50 border-l-4 border-amber-500 p-4 text-amber-700 text-sm flex items-start gap-3 rounded-r">
                <span className="text-xl">⚠️</span>
                <div>
                  <strong className="block mb-1">Clasificación Legacy (V1)</strong>
                  Este fondo aún no ha sido migrado a la taxonomía canónica V2. Los filtros, riesgos y exposiciones podrían ser inexactos.
                </div>
              </div>
            )}
            ...
            <section className="grid grid-cols-2 gap-8 border-t border-[#eeeeee] pt-8">
              ...
              <InfoRow label="Categoría" value={getCanonicalType(fund)} />
              <InfoRow label="Subcategoría" value={getCanonicalSubtype(fund)} />
              <InfoRow label="Región Principal" value={getCanonicalRegion(fund)} />
              
              {/* Etiqueta Custom: Suitability Low Risk (V2) */}
              {fund.classification_v2?.is_suitable_low_risk === false && (
                <div>
                    <div className="text-[10px] text-[#0B2545] uppercase font-bold tracking-[0.2em] mb-1">Perfil de Riesgo</div>
                    <span className="inline-flex items-center gap-1 px-2 py-1 bg-red-50 text-red-700 text-xs font-bold rounded border border-red-200">
                        🚫 Bloqueado en Perfiles Bajos
                    </span>
                </div>
              )}
              ...
              <div className="col-span-2 mt-2">
                <div className="text-[10px] text-[#0B2545] uppercase font-bold tracking-[0.2em] mb-2">
                  Etiquetas & Atributos (V2)
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  {getCanonicalFlags(fund).map(f => (
                    <span key={f} className="px-2 py-1 bg-[#0B2545]/5 text-[#0B2545] text-xs font-bold rounded shadow-sm border border-[#0B2545]/10">
                      {f}
                    </span>
                  ))}
                  {getCanonicalFlags(fund).length === 0 && <span className="text-xs text-slate-400 italic">No hay etiquetas adicionales</span>}
                </div>
              </div>
            </section>

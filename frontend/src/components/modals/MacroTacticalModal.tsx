...
export default function MacroTacticalModal({ portfolio, allFunds = [], numFunds = 6, onApply, onClose }: { portfolio: any[], allFunds?: any[], numFunds?: number, onApply: (p: any[]) => void, onClose: () => void }) {
    ...
    const chartData = {
        labels: ['USA', 'Zona Euro', 'Eur ex-Euro', 'China/Japón', 'Asia Em', 'Latam', 'Global', 'Sectorial', 'Gobierno', 'Corporativo', 'HY', 'Monetario', 'Alternativos'],
        datasets: [{
                ...
                <div className="p-6 overflow-y-auto scrollbar-thin grid grid-cols-12 gap-8">
                    ...
                    <div className="col-span-8 space-y-6">
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                <SliderRow label="Alternativos / Materias Primas" val={targets.commodities} onChange={v => handleSliderChange('commodities', v)} color="rose-500" />
                            </div>
                        </div>
                    </div>
                </div>

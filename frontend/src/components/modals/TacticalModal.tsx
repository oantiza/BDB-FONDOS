...
export default function TacticalModal({ currentPortfolio, proposedPortfolio, riskFreeRate = 0, onAccept, onClose, onSwap }: { currentPortfolio: any[], proposedPortfolio: any[], riskFreeRate?: number, onAccept: (p: any[]) => void, onClose: () => void, onSwap: (f: any) => void }) {
    ...
    const totalProposedWeight = editedProposal.reduce((acc, p) => acc + Number(p.weight || 0), 0)
    const totalCurrentWeight = currentPortfolio.reduce((acc, p) => acc + Number(p.weight || 0), 0)
        ...
        <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100">
                {portfolio.map(p => {
                    ...
                    const diff = orig ? Number(p.weight) - Number(orig.weight) : 0
                    ...
                    return (
                        <tr key={p.isin} className="hover:bg-slate-50 group transition-colors">
                            ...
                            <td className="p-2 text-right pr-4">
                                {readOnly ? (
                                    <span className="font-mono font-bold text-slate-400">{Number(p.weight || 0).toFixed(2)}%</span>
                                ) : (

...
export default function AnalysisModal({ portfolio, fundDatabase, onClose }) {
    ...
    const sortedHoldings = useMemo(() => {
        if (!metrics?.topHoldings && portfolio) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4 font-sans text-slate-700">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-7xl h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="p-5 border-b flex justify-between items-center bg-white text-brand shrink-0">
                    <h2 className="text-2xl font-bold flex items-center gap-2 text-[#0B2545]">
                        🔍 X-RAY Report
                    </h2>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white rounded border border-slate-200 p-4">
                                ...
                                <div className="w-full overflow-hidden">
                                    <CorrelationHeatmap matrix={metrics.correlationMatrix} assets={portfolio.map(p => p.name.substring(0, 15) + '...')} />
                                </div>
                            </div>

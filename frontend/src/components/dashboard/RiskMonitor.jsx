...
const MatrixCell = ({ active, color }) => (
    <div className={`w-full h-full rounded border border-slate-200 ${active ? color : 'bg-transparent'}`} />
)
            ...
            <div className="flex-1 flex flex-col gap-4 p-4">
                {risks.map((risk, idx) => (
                    <div key={idx} className="flex items-center justify-between">
                        <span className="text-xs text-slate-500 w-24">{risk.label}</span>
                        ...
                        <span className="text-[10px] font-bold text-slate-800 w-6 text-right">{risk.score}/5</span>
                    </div>
                ))}
            </div>
        </div>

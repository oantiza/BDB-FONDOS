...
export default function ComparatorRiskMap({ metricsA, metricsB, nameA, nameB }: ComparatorRiskMapProps) {
    ...
    const maxVol = Math.max(volA, volB) * 1.6 || 10;
        ...
        datasets: [
            {
                ...
                borderWidth: 3,
                pointRadius: 15, // Larger presence
                pointHoverRadius: 17,
    ...
    const options = {
        ...
        scales: {
            x: {
                title: { display: true, text: 'Riesgo (Volatilidad) %', font: { size: 13, weight: 'bold' as const } },
                grid: { color: '#f1f5f9' },
                ticks: { font: { size: 12 } },
                min: 0,
                max: maxVol
            ...
            y: {
                title: { display: true, text: 'Retorno Anual (CAGR) %', font: { size: 13, weight: 'bold' as const } },
                grid: { color: '#f1f5f9' },
                ticks: { font: { size: 12 } },

            ...
            <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full p-6 max-h-[90vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="border-2 border-gray-200 rounded-lg p-4 bg-gray-50 opacity-75">
                        ...
                        <p className="text-sm text-gray-500 mb-4">{originalFund.std_extra.company}</p>
                        
                        <div className="bg-white p-3 rounded border mb-4">
                            <StatRow label="Volatilidad" value={(originalFund.std_perf.volatility * 100).toFixed(2)} isPercentage />
                            <StatRow label="Sharpe" value={originalFund.std_perf.sharpe.toFixed(2)} />
                            <StatRow label="Coste (TER)" value={originalFund.std_extra.ter} isPercentage />
                        </div>
                    ...
                    {alternatives.map((alt: any) => {
                        ...
                        return (
                            <div key={alt.fund.isin} className="border-2 border-blue-100 hover:border-blue-500 rounded-lg p-4 bg-white transition-all shadow-sm hover:shadow-md transform hover:-translate-y-1">
                                ...
                                <p className="text-sm text-gray-500 mb-4">{alt.fund.std_extra.company}</p>
                                
                                <div className="bg-blue-50 p-3 rounded border border-blue-100 mb-4">
                                    <StatRow 
                                        label="Volatilidad" 
                                        value={(alt.fund.std_perf.volatility * 100).toFixed(2)} 
                                        isPercentage 
                                    />
                                    <StatRow 
                                        label="Sharpe" 
                                        value={alt.fund.std_perf.sharpe.toFixed(2)} 
                                        delta={alt.fund.std_perf.sharpe - originalFund.std_perf.sharpe} 
                                    />
                                    <StatRow 
                                        label="Ahorro Coste" 

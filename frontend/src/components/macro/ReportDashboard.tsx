...
import { TrendingUp, TrendingDown, AlertTriangle, Info, ArrowUpRight, ArrowDownRight, Minus, Calendar, Globe, Target, AlertCircle, BookOpen, Clock, Activity, ShieldAlert, BarChart3, LayoutGrid, Quote, Printer } from 'lucide-react';
...
export default function ReportDashboard({ reportData }: ReportDashboardProps) {
    ...
    const handlePrint = () => {
        if (reportData) {
            localStorage.setItem('current_macro_report', JSON.stringify(reportData));
        }
        window.open('/print/macro-report', '_blank');
    };

    return (
        <div className="w-full bg-[#f8fafc] text-slate-800 font-sans selection:bg-indigo-100 selection:text-indigo-900 pb-20 pt-8 print:bg-white print:pb-0">
            <style>{`
                @media print {
                    ...
                    /* Force all tabs content to be visible or focus on active one? 
                       Specifically for this dashboard, we want the current view but ideally 
                       if they want the full report, they should be in 'full' tab. 
                       However, let's make the container a simple block for printing. */
                    .max-w-7xl { max-width: 100% !important; padding: 0 !important; margin: 0 !important; }
                    .bg-white { border: none !important; box-shadow: none !important; }
                    .rounded-3xl, .rounded-2xl, .rounded-xl { border-radius: 0 !important; }
                    
                    /* Ensure Charts are visible */
                    .recharts-responsive-container { min-width: 100% !important; page-break-inside: avoid; }
                    
                    /* Hierarchy improvements for print */
                    h1 { color: #000 !important; }
                    h2, h3 { color: #1a1a1a !important; page-break-after: avoid; }
                    p { color: #333 !important; }
                    
                    /* Avoid breaking in the middle of a card or section */
                    .grid > div, article, table, .report-sections-container > div { page-break-inside: avoid; }
                }
            `}</style>
                            ...
                            <div className="border-b border-slate-200 pb-6 mb-12 flex flex-col md:flex-row justify-between items-end">
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 mt-2">
                                        <p className="text-slate-500 font-mono text-sm tracking-widest uppercase">{new Date().toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                                        <button
                                            onClick={handlePrint}
                                            className="flex items-center px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold rounded-full transition-all shadow-md hover:shadow-indigo-500/20 active:scale-95 no-print"
                                        >
                                            <Printer className="w-3 h-3 mr-1.5" /> IMPRIMIR PDF
                                        </button>
                                    </div>
                                </div>

...
export function auditFundClassification(fund: Fund): FundAuditTrail {
  const v2 = normalizeClassificationV2ForUI(fund.classification_v2);
  ...
  if (expV2?.economic_exposure) {
    ...
    signals.push({ source: 'portfolio_exposure_v2', field: 'equity_pct', value: ee.equity ?? null, weight: 'primary' });
    signals.push({ source: 'portfolio_exposure_v2', field: 'bond_pct', value: ee.bond ?? null, weight: 'primary' });
    signals.push({ source: 'portfolio_exposure_v2', field: 'cash_pct', value: ee.cash ?? null, weight: 'primary' });
  }
  ...
  if (v2 && (v2.classification_confidence ?? 0) < 0.5) {
...
function assessProfileEligibility(fund: Fund, profile: number): { eligible: boolean; reason: string } {
  const v2 = normalizeClassificationV2ForUI(fund.classification_v2);
  const exp = normalizePortfolioExposureV2ForUI(fund.portfolio_exposure_v2);
  ...
  if (profile <= 2) {
    if (v2.is_suitable_low_risk === false) {
    ...
    const realEq = Number(exp?.economic_exposure?.equity ?? 0);
    if (realEq > 30) {
      return { eligible: false, reason: `Real equity exposure ${realEq}% > 30%` };
    }
  }
  ...
  if (profile <= 4) {
    ...
    const realEq = Number(exp?.economic_exposure?.equity ?? 0);
    if (profile === 3 && realEq > 45) {
      return { eligible: false, reason: `Real equity ${realEq}% > 45% for profile 3` };
    }
    if (profile === 4 && realEq > 60) {
      return { eligible: false, reason: `Real equity ${realEq}% > 60% for profile 4` };
    }
  }
...
export function detectAmbiguousFunds(funds: Fund[]): AmbiguityReport {
  ...
  for (const fund of funds) {
    ...
    if (!v2) {
      ...
      if ((v2.classification_confidence ?? 0) < 0.5) {
        reasons.push(`Low confidence: ${v2.classification_confidence ?? 0}`);
      }
    ...
    if (!expV2) {
      ...
      if ((expV2.exposure_confidence ?? 0) < 0.5) {
        reasons.push(`Low exposure confidence: ${expV2.exposure_confidence ?? 0}`);
      }
    }

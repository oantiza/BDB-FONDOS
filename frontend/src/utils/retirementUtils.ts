...
export function getBonificacionTrabajo(_rendimientoNeto: number): number {
    // La normativa de Bizkaia NO utiliza reducciones por "rendimientos del trabajo" 
    // del tipo 8.000€ como en Territorio Común. Se compensa al final al nivel general 
    // de cuota mediante la minoración general y los tramos deflactados.
    return 0;
}
...
export interface RentTaxParams {
    ...
    ratioRentabilidadEpsvPre: number;
    ratioRentabilidadEpsvPost: number;
    isRentabilidadEPSVExenta: boolean;
...
export function calculateRentTaxes(params: RentTaxParams): RentTaxResult {
    ...
    const ratioRentabilidadEpsvPre = Math.min(1, Math.max(0, isFinite(params.ratioRentabilidadEpsvPre) ? params.ratioRentabilidadEpsvPre : 0));
    const ratioRentabilidadEpsvPost = Math.min(1, Math.max(0, isFinite(params.ratioRentabilidadEpsvPost) ? params.ratioRentabilidadEpsvPost : 0));
    ...
    // Aportación y Rentabilidad EPSV
    const rentabilidadEpsvPre = rentaAnualEPSV * ratioRentabilidadEpsvPre;
    const rentabilidadEpsvPost = rentaAnualEPSV * ratioRentabilidadEpsvPost;
    ...
    const cuotaTotalGeneral = calculateBizkaiaTaxBaseGeneral(pensionPublicaAnual + parteSujetaGeneralEPSV);
    const cuotaGeneralIncremental = Math.max(0, cuotaTotalGeneral - cuotaSoloPension);
    
    // IMPACTO AHORRO
    const cuotaAhorro = calculateBizkaiaTaxBaseAhorro(plusvaliaSujetaAhorros + parteSujetaAhorroEPSV);
    ...
    return {
        ...
        rentaAnualAhorros,
        parteExentaEPSV,
        parteSujetaEPSV: parteSujetaGeneralEPSV + parteSujetaAhorroEPSV,
        plusvaliaSujetaAhorros,
        parteExentaAhorros,
        totalExento,
        totalSujetoGeneral: parteSujetaGeneralEPSV,
        totalSujetoAhorro: plusvaliaSujetaAhorros + parteSujetaAhorroEPSV,

...
export function calcSimpleStats(
    ...
    let finalVol = (realVol !== null && isFinite(realVol) && !isNaN(realVol)) ? realVol : wVol * diversificationFactor;

    // VALIDATION: If volatility is unreasonably low (< 0.01%) or NaN, invalidate it.
    // This handles cases where data might be missing or effectively zero.
    if (!isFinite(finalVol) || isNaN(finalVol) || finalVol < 0.0001) {

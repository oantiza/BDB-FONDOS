...
export function getCanonicalType(fund: any): string {
    if (fund?.classification_v2?.asset_type) {
        return fund.classification_v2.asset_type;
    }
...
export function getCanonicalRegion(fund: any): string {
    if (fund?.classification_v2?.region_primary) {
        return fund.classification_v2.region_primary;
    }
    return fund?.primary_region || fund?.std_region || 'Desconocido';
}

export function getCanonicalRiskBucket(fund: any): string | null {
    if (fund?.classification_v2?.risk_bucket) {
        return fund.classification_v2.risk_bucket;
    }
    return null;
}
...
export function getCanonicalFlags(fund: any): string[] {
    const flags: string[] = [];
    if (!fund?.classification_v2) return flags;

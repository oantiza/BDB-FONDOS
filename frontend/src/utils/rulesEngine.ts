...
// ============================================================================
// UNIFIED SCORING (Frontend + Backend Consistent)
// ============================================================================
function calculateScore(fund, targetVol = 0.12) {
    ...
    // 5. Quality (10% - Calidad de datos)
    const qualityScore = Math.min(history / 10, 1) * 10;

    // 6. MAX DRAWDOWN PENALTY (NEW - P2 Integration)
    const maxDrawdown = fund.std_perf?.max_drawdown || fund.perf?.max_drawdown || 0;
    ...
    const sortino = fund.std_perf?.sortino_ratio || fund.perf?.sortino_ratio || 0;
    let sortinoBonus = 0;
    if (sortino > 0) {
        // Bonus for good downside risk management
        const sortinoNorm = Math.max(0, Math.min(4, sortino));
        sortinoBonus = (sortinoNorm / 4) * 5; // Max 5 points
    }
...
function getPoolOfCandidates(riskLevel, profile, fundDatabase, minCount = 5) {
    ...
    // Puntuar y ordenar - USAR targetVol del perfil de riesgo

...
import XRayReportGenerator from '../components/xray/XRayReportGenerator';
...
export default function XRayPage({ portfolio, fundDatabase, totalCapital, onBack }: XRayPageProps) {
    ...
    const [showPatrimonial, setShowPatrimonial] = useState(false);
        ...
        <div className="flex flex-col h-screen bg-[#f8fafc]">
                    ...
                    headerActions={
                        hasMetrics ? (
                            <div className="flex items-center gap-3 border-r border-slate-600/50 pr-4">
                                ...
                                <button
                                    onClick={() => setShowPatrimonial(true)}
                                    className="bg-slate-700 hover:bg-slate-600 text-white transition-colors text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded border border-slate-500 flex items-center gap-2 shadow-sm"
                                >
                                    <span>⚖️</span> Comparador Patrimonial
                                </button>
                            </div>
                        ) : undefined
                    }
            ...
            <div className="flex-1 overflow-y-auto p-8 relative mt-16">
                <div className="max-w-[1400px] mx-auto bg-white p-8 shadow-sm border border-slate-100 min-h-[1000px]">
                    ...
                    {/* SECTION 5: REPORT GENERATOR (MOVED TO MODAL) */}
            ...
            {/* COMPARADOR PATRIMONIAL MODAL */}
            {showPatrimonial && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-8 backdrop-blur-sm overflow-y-auto">
                    <div className="w-full max-w-[1200px] relative">
                         <div className="flex justify-end mb-4">
                             <button onClick={() => setShowPatrimonial(false)} className="text-white hover:text-slate-200 bg-slate-800/50 hover:bg-slate-800 transition-colors px-4 py-2 rounded-full font-bold text-sm tracking-wider uppercase flex items-center gap-2">✕ Cerrar Comparador</button>
                         </div>
                         <div className="bg-[#f8fafc] rounded-xl shadow-2xl overflow-y-auto max-h-[85vh] p-2 relative">
                             <XRayReportGenerator />
                         </div>
                    </div>
                </div>

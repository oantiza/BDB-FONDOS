...
import { Search, X, TrendingUp, Activity, AlertTriangle, ArrowLeft, Star } from 'lucide-react';
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin p-6">
                <div className="flex justify-between items-end mb-6">
                    <div>
                        ...
                        <p className="text-lg text-slate-500 mt-1 ml-12">Analiza y compara el rendimiento histórico de tus fondos favoritos</p>
                    </div>
                    <div className="flex bg-slate-100 rounded-lg p-1 gap-1">
                        {[1, 3, 5, 10].map((y) => (
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-md text-base font-bold transition ${period === y ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-200'
                                    }`}
                            >
                                {y}Y
                            </button>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4 mb-6">
                    <div className="flex gap-4">
                        ...
                        <div className="w-48 space-y-1">
                            ...
                            <select
                                className="w-full bg-slate-50 border-slate-200 border rounded-lg p-2.5 text-base text-slate-700 focus:ring-2 focus:ring-blue-500 outline-none"
                        ...
                        <div className="w-48 space-y-1">
                            <label className="text-sm font-bold text-slate-500 uppercase">Región</label>
                            <select
                                className="w-full bg-slate-50 border-slate-200 border rounded-lg p-2.5 text-base text-slate-700 focus:ring-2 focus:ring-blue-500 outline-none"
                                value={regionFilter}
                        ...
                        <div className="flex-1 space-y-1 relative">
                            <label className="text-sm font-bold text-slate-500 uppercase">Añadir Fondo</label>
                            <div className="relative">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                                <input
                                    className="w-full bg-slate-50 border-slate-200 border rounded-lg py-2.5 pl-10 pr-4 text-base text-slate-700 placeholder-slate-400 focus:ring-2 focus:ring-blue-500 outline-none transition-all focus:bg-white"
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                        {selectedFunds.map((f, i) => (
                            <div key={f.id} className="flex items-center gap-2 bg-blue-50 pl-3 pr-2 py-1.5 rounded-full border border-blue-100 group shadow-sm" style={{ borderLeft: `3px solid ${COLORS[i % COLORS.length]}` }}>
                                <div className="text-lg font-medium text-blue-900 max-w-[200px] truncate" title={f.name}>{f.name}</div>
                                <button onClick={() => removeFund(f.id)} className="p-1 hover:bg-red-100 hover:text-red-500 rounded-full transition text-blue-300"><X size={16} /></button>
                            </div>
                        ))}
                        {selectedFunds.length === 0 && <span className="text-slate-400 text-lg italic py-2">Selecciona fondos para comparar...</span>}
                    </div>
                </div>
                ...
                {selectedFunds.length > 0 && (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative flex-1 min-h-0">
                        ...
                        <div className="lg:col-span-3 bg-white rounded-xl border border-slate-200 p-6 shadow-sm h-[400px] flex flex-col">
                            <h3 className="text-xl font-bold text-slate-700 mb-4 flex items-center gap-2"><Activity size={20} className="text-slate-400" /> Performance Relativa (Rebase 100)</h3>
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={chartData}>
                                        ...
                                        <XAxis
                                            ...
                                            minTickGap={50}
                                        />
                                        <YAxis stroke="#94a3b8" fontSize={14} domain={['auto', 'auto']} />
                                        <Tooltip
                        ...
                        <div className="lg:col-span-3 bg-white rounded-xl border border-slate-200 p-6 shadow-sm overflow-hidden">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-xl font-bold text-slate-700 flex items-center gap-2"><AlertTriangle size={20} className="text-slate-400" /> Métricas de Riesgo y Retorno</h3>
                                <span className="text-sm text-slate-400">* Sharpe calculado sobre 3 años (RF 2%). Volatilidad anualizada.</span>
                            </div>
                                ...
                                <table className="w-full text-left text-base">
                                    <thead className="bg-slate-50 text-slate-500 border-b border-slate-200">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 text-right text-emerald-600 font-semibold">Retorno 1Y</th>
                                            <th className="px-4 py-3 text-right text-emerald-600 font-semibold">Retorno 3Y</th>
                                            <th className="px-4 py-3 text-right text-emerald-600 font-semibold">Retorno 5Y</th>
                                            <th className="px-4 py-3 text-right text-emerald-600 font-semibold">Retorno 10Y</th>
                                            <th className="px-4 py-3 text-center text-amber-500 font-semibold">Rating</th>
                                            ...
                                            <th className="px-4 py-3 text-right text-red-600 font-semibold rounded-tr-lg">Max DD 3Y</th>
                                        </tr>
                                    </thead>

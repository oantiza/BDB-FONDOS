...
const updateRow = (id, oldV, newV, fmt) => {
    ...
    if (elDiff) {
        ...
        // Logic Color: Green is good (more return, less risk)
        let isGood = diff > 0;
        if (id === 'vol') isGood = diff < 0; // Less vol is good
...
async function runOptimizationWithComparison() {
    ...
    try {
        ...
        const riskVal = riskInput ? parseInt(riskInput.value) : 5;

        // 1. Generate Proposal (Local Quant)
        proposedPortfolio = generateSmartPortfolio(riskVal);

        if (!proposedPortfolio.length) throw new Error("No se pudo generar una cartera óptima.");

        // 2. Open Modal
        openModal(document.getElementById('fund-comparison-modal'));

        // 3. Render Comparisons
        const statsOld = calcSimpleStats(currentPortfolio);
        const statsNew = calcSimpleStats(proposedPortfolio);

        updateRow('ret', statsOld.ret, statsNew.ret, (v) => (v * 100).toFixed(2) + '%');
        updateRow('vol', statsOld.vol, statsNew.vol, (v) => (v * 100).toFixed(2) + '%');

        const sharpeOld = statsOld.vol > 0 ? (statsOld.ret - 0.02) / statsOld.vol : 0;
        const sharpeNew = statsNew.vol > 0 ? (statsNew.ret - 0.02) / statsNew.vol : 0;
        updateRow('sharpe', sharpeOld, sharpeNew, (v) => v.toFixed(2));

        // Charts
        renderComparisonChart(
            generateProjectionPoints(statsOld.ret, statsOld.vol),
            generateProjectionPoints(statsNew.ret, statsNew.vol),
            'comp-chart-optimized' // Reusing logic but maybe needs split charts? 
            // Validar IDs en modal: comp-chart-current, comp-chart-optimized
        );
        // NOTE: renderComparisonChart might write to 1 canvas. 
        // We need 2 separate charts logic or renderComparisonChart handles 'comp-chart-optimized' only?
        // Let's rely on chartEngine.js having renderDashboardChart or similar.
        // Actually, let's just render standard Donut or simple projection.
        // For now, render Risk Map or simpler.

        renderBar('comp-bar-old', currentPortfolio);
        renderBar('comp-bar-new', proposedPortfolio);

        // Bind Apply Button
        const applyBtn = document.getElementById('btn-apply-optimization');
        if (applyBtn) {
            applyBtn.onclick = () => {
                currentPortfolio = [...proposedPortfolio];
                updatePortfolioUI();
                document.getElementById('fund-comparison-modal').classList.add('hidden');
...
window.runOptimizationWithComparison = runOptimizationWithComparison;

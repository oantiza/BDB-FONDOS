...
export function openModal(modal, fundIsin = null) {
    // ROBUSTEZ: Si nos pasan un string (ID), buscamos el elemento. Si es elemento, lo usamos.
...
function loadFundDetailsIntoModal(isin) {
    ...
    // Selectores seguros
    const elName = document.getElementById('fd-name'); if(elName) elName.textContent = fund.name;
    const elIsin = document.getElementById('fd-isin'); if(elIsin) elIsin.textContent = fund.isin;
    const elClass = document.getElementById('fd-class'); if(elClass) elClass.textContent = fund.type || inferType(fund);
    
    const vol = fund.perf?.volatility || 0;
    const elVol = document.getElementById('fd-vol'); if(elVol) elVol.textContent = vol.toFixed(2) + '%';
...
// EXPORTAR A WINDOW para HTML events
window.addVip = (isin) => { 

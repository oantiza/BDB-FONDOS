/**
 * Reparse Morningstar PDFs -> Firestore funds_v3 (Gemini 2.5 Pro por defecto)
 ...
 *   --processed <dir>           (default ./PROCESADOS)
 *   --error <dir>               (default ./ERRORES)
 *   --backup-root <dir>         (default ./BDB_PARSE_BACKUP)
...
const MODEL_NAME = getArgValue("--model") || "gemini-2.5-pro";
...
const BACKUP_ROOT = path.resolve(getArgValue("--backup-root") || path.join(__dirname, "BDB_PARSE_BACKUP"));

const DIRS = {
  INPUT: path.join(BACKUP_ROOT, "00_input_pdfs"),
  RAW_TEXT: path.join(BACKUP_ROOT, "01_raw_text"),
  RAW_LLM: path.join(BACKUP_ROOT, "02_raw_llm"),
  PARSED: path.join(BACKUP_ROOT, "03_parsed_ms"),
  CANONICAL: path.join(BACKUP_ROOT, "04_canonical"),
  REVIEW: path.join(BACKUP_ROOT, "05_overrides", "review_queue"),
  EXPORTS: path.join(BACKUP_ROOT, "06_exports"),
  MANIFESTS: path.join(BACKUP_ROOT, "07_manifests"),
  LOGS: path.join(BACKUP_ROOT, "08_logs"),
  ERRORS: path.join(BACKUP_ROOT, "08_logs", "errors"),
};
...
function validateRawLlMSchema(msRaw) {
  ...
  for (const k of Object.keys(msRaw)) {
    if (!allowedTopLevel.has(k)) {
      errors.push(`unexpected_top_level_key:${k}`);
    }
  }
  ...
  for (const k of scalarOrNull) {
    if (msRaw[k] !== undefined && msRaw[k] !== null && typeof msRaw[k] !== "string") {
      errors.push(`invalid_type:${k}:expected_string_or_null`);
    }
  }
  ...
  for (const k of numOrNull) {
    if (msRaw[k] !== undefined && msRaw[k] !== null && !Number.isFinite(parseNum(msRaw[k]))) {
      errors.push(`invalid_type:${k}:expected_number_or_null`);
    }
  }
  ...
  for (const k of objOrNull) {
    if (msRaw[k] !== undefined && msRaw[k] !== null && !isPlainObject(msRaw[k])) {
      errors.push(`invalid_type:${k}:expected_object_or_null`);
    }
  }

  if (msRaw.holdings_top10 !== undefined && msRaw.holdings_top10 !== null && !Array.isArray(msRaw.holdings_top10)) {
    errors.push("invalid_type:holdings_top10:expected_array_or_null");
  }
...
function decidePipelineStatus({ schemaValidation, canonicalValidation, classification_v2, portfolio_exposure_v2 }) {
  ...
  const allWarnings = [...warnings, ...classWarnings, ...expWarnings];
  ...
  const needsReview = allWarnings.some((w) => reviewTriggers.some((t) => String(w).includes(t)));
...
function deriveAssetSubtype(catUpper, subcats = [], nameUpper = "") {
  ...
  if (c.includes("BOND") || c.includes("CREDIT") || c.includes("FIXED INCOME") || c.includes("RENTA FIJA")) {
...
async function processPdfFile(fileName, writer) {
  ...
  const pdfBackupPath = path.join(DIRS.INPUT, `${stableBaseName}.pdf`);
  ...
  if (!fs.existsSync(pdfBackupPath)) {
    fs.copyFileSync(fullPath, pdfBackupPath);
  }
  ...
  if (fi) {
    ...
    if (ig > 60 && hy < 20) creditBucket = "investment_grade";
    else if (hy > 35) creditBucket = "high_yield";
    else if (ig > 20 || hy > 20) creditBucket = "mixed_credit";

    if (fiText.includes("HIGH YIELD") || fiText.includes("ALTO RENDIMIENTO")) {
      fixedIncomeType = "high_yield";
      if (!creditBucket) creditBucket = "high_yield";
    ...
    } else if (fiText.includes("CONVERTIBLE")) {
  ...
  const eqV2 = clampPct(ms.portfolio?.asset_allocation?.equity);
  const fiV2 = clampPct(ms.portfolio?.asset_allocation?.bond);
  const cashV2 = clampPct(ms.portfolio?.asset_allocation?.cash);
  const otherV2 = clampPct(ms.portfolio?.asset_allocation?.other);

  if (eqV2 !== null || fiV2 !== null || cashV2 !== null || otherV2 !== null) {
    portfolio_exposure_v2 = {
      version: "v2",
      asset_mix: {
        equity: eqV2 !== null ? +(eqV2 / 100).toFixed(6) : 0,
        bond: fiV2 !== null ? +(fiV2 / 100).toFixed(6) : 0,
        cash: cashV2 !== null ? +(cashV2 / 100).toFixed(6) : 0,
        other: otherV2 !== null ? +(otherV2 / 100).toFixed(6) : 0,
      },
      equity_regions: equity_regions_total,
      sectors: equity_sectors_total,
      equity_styles: style_weights_total,
      market_caps: size_weights_total,
      bond_types: fixedIncomeType && fiV2 !== null ? { [fixedIncomeType]: +(fiV2 / 100).toFixed(6) } : null,
      credit: creditBucket && fiV2 !== null ? { [creditBucket]: +(fiV2 / 100).toFixed(6) } : null,
      duration: durationBucket && fiV2 !== null ? { [durationBucket]: +(fiV2 / 100).toFixed(6) } : null,
      alternatives:
        classification_v2.asset_type === "alternative" && otherV2 !== null
          ? { alternative: +(otherV2 / 100).toFixed(6) }
          : null,
  ...
  if (routing.status === PIPELINE_STATUS.OK) {
    ...
    throw new Error(`${routing.status}: ${routing.reason || "unknown reason"}`);
  }
  ...
  return { isin, fileName, routingStatus: routing.status };
}

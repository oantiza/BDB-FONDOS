...
def get_price_data(assets_list, db):
    price_data = {}
    print(f"📥 Buscando precios para {len(assets_list)} activos...")
    
    for i, isin in enumerate(assets_list):
        loaded = False
        try:
...
def optimize_portfolio_quant(request: https_fn.CallableRequest):
    try:
        ...
        # CORRECCIÓN: Límite dinámico basado en número de activos
        n_assets = len(assets_list)
        # Si hay pocos activos, el límite debe ser mayor para permitir sumar 100%
        # Ejemplo: Si hay 2 activos, min 50% cada uno. Ponemos límite 60% para dar juego.
        # Si hay muchos, mantenemos el 40% o bajamos.
        max_weight_limit = max(0.40, (1.0 / n_assets) + 0.15)
        
        ef = EfficientFrontier(mu, S)
        ef.add_objective(objective_functions.L2_reg, gamma=0.5) 
        ef.add_constraint(lambda w: w <= max_weight_limit) 
...
def backtest_portfolio(request: https_fn.CallableRequest):
    try:
        ...
        if not valid_assets: raise Exception("Sin datos válidos")
        ...
        days = len(cumulative)
        years = days / 252
        total_ret = cumulative.iloc[-1] / 100 - 1
        cagr = (1 + total_ret) ** (1/years) - 1 if years > 0 else 0
        vol = port_ret.std() * np.sqrt(252)
        sharpe = (cagr - 0.03) / vol if vol > 0 else 0
        max_dd = ((cumulative - cumulative.cummax()) / cumulative.cummax()).min()
        
        rf_curve = df[BENCHMARK_RF_ISIN] if BENCHMARK_RF_ISIN in df else pd.Series(np.linspace(100, 110, len(cumulative)), index=cumulative.index)
        rv_curve = df[BENCHMARK_RV_ISIN] if BENCHMARK_RV_ISIN in df else pd.Series(np.linspace(100, 150, len(cumulative)), index=cumulative.index)
        
        def normalize(ser): return (ser / ser.iloc[0] * 100)
...
def getFinancialNews(request: https_fn.CallableRequest):
    try:
        ...
        if isinstance(news_list, dict): news_list = news_list.get('data', [])
...
def generate_daily_json(request):
    return https_fn.Response("Use local generator.", status=200)
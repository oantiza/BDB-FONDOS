 ...
 *
 * Aplica overrides manuales institucionales sobre canonical.
 *
 * Entrada:
 *   data/canonical/*.json
 *   overrides/05_overrides/*.override.json
 *
 * Salida:
 *   data/work/canonical_applied/*.json
 *   data/work/manifests/overrides_manifest.json
 *   data/work/logs/overrides_errors.ndjson
 *
 * Reglas:
...
function resolvePreferredOrLegacy(preferredPath, ...legacyPaths) {
...
const WORK_ROOT = path.join(DATA_ROOT, "work");
const LEGACY_BACKUP_ROOT = path.join(ROOT, "BDB_PARSE_BACKUP");
const CANONICAL_DIR = path.resolve(
  getArgValue("--canonical") ||
    resolvePreferredOrLegacy(
      path.join(DATA_ROOT, "canonical"),
      path.join(ROOT, "04_canonical"),
      path.join(LEGACY_BACKUP_ROOT, "04_canonical")
    )
);
const OVERRIDES_DIR = path.resolve(
  getArgValue("--overrides") ||
    resolvePreferredOrLegacy(
      path.join(ROOT, "overrides", "05_overrides"),
      path.join(ROOT, "05_overrides"),
      path.join(LEGACY_BACKUP_ROOT, "05_overrides")
    )
);
const OUTPUT_DIR = path.resolve(
  getArgValue("--output") ||
    resolvePreferredOrLegacy(
      path.join(WORK_ROOT, "canonical_applied"),
      path.join(ROOT, "04_canonical_applied"),
      path.join(LEGACY_BACKUP_ROOT, "04_canonical_applied")
    )
);
const MANIFEST_DIR = path.resolve(
  getArgValue("--manifest-dir") || resolvePreferredOrLegacy(path.join(WORK_ROOT, "manifests"), path.join(LEGACY_BACKUP_ROOT, "07_manifests"))
);
const LOGS_DIR = path.resolve(
  getArgValue("--logs-dir") || resolvePreferredOrLegacy(path.join(WORK_ROOT, "logs"), path.join(LEGACY_BACKUP_ROOT, "08_logs"))
);

...
const BATCH_OFFSET = 25; // skip first 25 (already written in batch 0)
const BATCH_INDEX = 1;
...
const OUT_DIR = path.join(BASE, "artifacts", "morningstar_pdf_updated_batch", "write_controlled_100_1");
const OUT_BATCH_MANIFEST  = path.join(OUT_DIR, "batch_manifest.json");
const OUT_ROLLBACK        = path.join(OUT_DIR, "rollback_batch_100_1.json");
const OUT_POSTWRITE       = path.join(OUT_DIR, "postwrite_verification.json");
const OUT_SUMMARY         = path.join(OUT_DIR, "write_summary.json");
const OUT_REPORT          = path.join(BASE, "docs", "BDB_MORNINGSTAR_PDF_UPDATED_BATCH_WRITE_CONTROLLED_100_1.md");
...
async function main() {
  ...
  console.log("BDB-MORNINGSTAR-PDF-UPDATED-BATCH-WRITE-CONTROLLED-100-1");
  console.log(`MODE: WRITE CONTROLLED — ${BATCH_SIZE} documents (offset ${BATCH_OFFSET})`);
  console.log("=".repeat(60));
  console.log();
  ...
  const batchManifest = {
    ...
    task: "BDB-MORNINGSTAR-PDF-UPDATED-BATCH-WRITE-CONTROLLED-100-1",
  ...
  console.log(`   rollback_batch_100_1.json written (${BATCH_SIZE} docs)`);
  ...
  // ── 7: WRITE ──
  console.log(`[7/10] EXECUTING WRITES — ${BATCH_SIZE} documents...`);
  let writeCount = 0;
  const writeResults = [];
  ...
  for (const entry of batch) {
    ...
    try {
      ...
      if (writeCount % 10 === 0 || writeCount === BATCH_SIZE) {
        console.log(`   [${writeCount}/${BATCH_SIZE}] last: ${isin} — OK`);
      }
    } catch (err) {
      writeResults.push({ isin, status: "ERROR", error: err.message });
      console.error(`   [${writeCount + 1}/${BATCH_SIZE}] ${isin} — ERROR: ${err.message}`);
    }
  }
  ...
  for (const entry of batch) {
    ...
    if (preManual !== undefined) {
      ...
      else { checks.issues.push("manual MODIFIED — CRITICAL"); checks.status = "CRITICAL_FAILURE"; }
    } else {
    ...
    if (preCosts !== undefined) {
      ...
      else { checks.issues.push("manual.costs MODIFIED — CRITICAL"); checks.status = "CRITICAL_FAILURE"; }
    }
    ...
    if (preRetro !== undefined) {
      ...
      else { checks.issues.push("manual.costs.retrocession MODIFIED — CRITICAL"); checks.status = "CRITICAL_FAILURE"; }
    }
  ...
  const writeSummary = {
    ...
    cumulative: { total_written: 25 + writeCount, total_remaining: 520 - 25 - writeCount },
    rollback_available: "rollback_batch_100_1.json",
  ...
  const batchRows = batch.map((e, i) => {
    ...
    return `| ${BATCH_OFFSET + i + 1} | \`${e.isin}\` | ${e.name} | ${e.asset_class || "—"} | ${wr ? wr.status : "—"} | ${vr ? vr.status : "—"} |`;
  }).join("\n");
  ...
  const recommendation = criticals.length > 0
    ...
    : warns.length > 0
      ...
      : `> [!TIP]\n> Los ${BATCH_SIZE} writes se ejecutaron correctamente. manual.* intacto en todos los casos.\n\n**Recomendacion**: Proceder con el batch restante de **${520 - 25 - writeCount} ISINs**.`;

  const md = `# BDB Morningstar PDF Updated Batch Write Controlled 100-1
...
| **Batch 1 (100-1)** | **${BATCH_SIZE}** | **ESTE BATCH** |
| Restantes | ${520 - 25 - writeCount} | Pendiente |
| Missing (excluidos) | 2 | LU0171281750, LU0171282212 |
| **Total escrito** | **${25 + writeCount}** | |
...
| REVIEW ISINs | ${reviewSet.size} | SI — excluidos |
| ERROR ISINs | ${errorSet.size} | SI — excluidos |
| Batch 0 (ya escritos) | 25 | SI — offset ${BATCH_OFFSET} |
...
| Campo | Estado |
|:---|:---|
| \`manual\` | **${criticals.length === 0 ? "INTACTO — CONFIRMADO" : "CRITICAL FAILURE"}** |
| \`manual.costs\` | **${criticals.length === 0 ? "INTACTO — CONFIRMADO" : "CRITICAL FAILURE"}** |
| \`manual.costs.retrocession\` | **${criticals.length === 0 ? "INTACTO — CONFIRMADO" : "CRITICAL FAILURE"}** |
...
| manual.* tocado | **${criticals.length === 0 ? "NO — CONFIRMADO" : "CRITICAL"}** |
| manual.costs.retrocession tocado | **${criticals.length === 0 ? "NO — CONFIRMADO" : "CRITICAL"}** |
...
*Batch 1 (100 fondos) — Writes: ${writeCount} — Total acumulado: ${25 + writeCount}/520*
`;
  ...
  console.log();
  console.log("=".repeat(60));
  console.log("WRITE CONTROLLED 100-1 SUMMARY");
  console.log("=".repeat(60));
  ...
  console.log(`  Cumulative:  ${25 + writeCount}/520 written`);
  console.log(`  Remaining:   ${520 - 25 - writeCount}`);
  console.log(`  Elapsed:     ${Date.now() - startTime.getTime()} ms`);
  console.log("=".repeat(60));
  console.log(criticals.length > 0
    ? "STATUS: CRITICAL FAILURES — STOP"
    : "STATUS: WRITE CONTROLLED 100-1 COMPLETE — ALL OK");

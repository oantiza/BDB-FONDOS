...
PURPOSE: Final stable script for populating/fixing Taxonomy V2 data in Firestore.
SAFE_MODE: MUTATES_FIRESTORE
RUN: python scripts/maintenance/populate_taxonomy_v2.py
"""
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def _clamp01(value: float) -> float:
    return round(_clamp(float(value), 0.0, 1.0), 4)
...
def _normalize_pct_block(eq: float, bd: float, ca: float, oth: float):
    total = eq + bd + ca + oth
    if total <= 0:
...
def _normalize_percent_triplet(a: float, b: float, c: float):
    total = a + b + c
    if total <= 0:
        return 0.0, 0.0, 0.0
...
def _deduce_region_primary(data: dict, asset_type: AssetClassV2) -> tuple[RegionV2, float]:
    ...
    if macro or detail:
        ...
        if asset_type == AssetClassV2.EQUITY:
            if us >= 25 and europe_total >= 25:
                return RegionV2.GLOBAL, 0.86
...
def _deduce_market_cap_bias(ms_style: dict, data: dict) -> tuple[MarketCapBiasV2, bool]:
    ms_style = _safe_dict(ms_style)
    mcap_data = _safe_dict(ms_style.get("market_cap"))
    large = _safe_float(mcap_data.get("giant", 0)) + _safe_float(
        mcap_data.get("large", 0)
    )
    mid = _safe_float(mcap_data.get("mid", 0))
    small = _safe_float(mcap_data.get("small", 0)) + _safe_float(
        mcap_data.get("micro", 0)
    )
...
def classifyFundV2(isin: str, data: dict) -> ClassificationV2:
    ...
    klass.source_priority_used = _dedupe_list(klass.source_priority_used)
    return klass
...
def buildPortfolioExposureV2(isin: str, data: dict, klass: ClassificationV2) -> PortfolioExposureV2:
    ...
    if style_data and cap_data:
        ...
        g = _safe_float(style_data.get("growth", 0))

        lg, md, sm = _normalize_percent_triplet(lg, md, sm)
        v, b, g = _normalize_percent_triplet(v, b, g)

        exp.equity_styles = {
            "large_growth": round((lg * g) / 100.0, 2),
            "large_value": round((lg * v) / 100.0, 2),
            "large_core": round((lg * b) / 100.0, 2),
            "mid_growth": round((md * g) / 100.0, 2),
            "mid_value": round((md * v) / 100.0, 2),
            "mid_core": round((md * b) / 100.0, 2),
            "small_growth": round((sm * g) / 100.0, 2),
            "small_value": round((sm * v) / 100.0, 2),
            "small_core": round((sm * b) / 100.0, 2),
        }
...
def test_math_logic():
    ...
    samples = [
        ...
        {
            "label": "Global infrastructure equity",
            "data": {
                "name": "FTGF ClearBridge Infrastructure Value Fund Class A Euro Accumulating",
                "ms": {
                    ...
                    "regions": {
                        ...
                        "detail": {
                            ...
                            "canada": 7.15,
                            "eurozone": 37.26,
                            "united_kingdom": 10.37,
                            "latin_america": 1.47,
                        },
                    },
                },
                "metrics": {"equity": 97.27},
            },
        },
        {
            "label": "Eurobolsa should be Europe Equity",
            "data": {
                "name": "EUROBOLSA SELECCION SICAV, SA",
                "legacy_asset_class": "Otros",
            },
        },
        {
            "label": "MSCI World Index should be Global Equity",
            "data": {
                "name": "Fidelity MSCI World Index Fund EUR P Acc (Hedged)",
                "legacy_asset_class": "Otros",
            },
        },
    ]
    ...
    for s in samples:
        ...
        print(
            ...
            f"risk={klass.risk_bucket.value} | "
            f"low_risk={klass.is_suitable_low_risk} | "
        ...
        print(
            ...
            f"cash={exp.economic_exposure.cash} "
            f"other={exp.economic_exposure.other}"
        )
        if klass.warnings:
            print("warnings=", klass.warnings)
        print()

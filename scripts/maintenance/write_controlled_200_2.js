 ...
 *   node scripts/maintenance/write_controlled_200_2.js
 */
...
const BATCH_OFFSET = 125; // skip first 25 (already written in batch 0)
const BATCH_INDEX = 2;
...
const OUT_DIR = path.join(BASE, "artifacts", "morningstar_pdf_updated_batch", "write_controlled_200_2");
const OUT_BATCH_MANIFEST  = path.join(OUT_DIR, "batch_manifest.json");
const OUT_ROLLBACK        = path.join(OUT_DIR, "rollback_batch_200_2.json");
const OUT_POSTWRITE       = path.join(OUT_DIR, "postwrite_verification.json");
const OUT_SUMMARY         = path.join(OUT_DIR, "write_summary.json");
const OUT_REPORT          = path.join(BASE, "docs", "BDB_MORNINGSTAR_PDF_UPDATED_BATCH_write_controlled_200_2.md");
...
async function main() {
  ...
  console.log("BDB-MORNINGSTAR-PDF-UPDATED-BATCH-WRITE-CONTROLLED-200-2");
  ...
  const batchManifest = {
    ...
    task: "BDB-MORNINGSTAR-PDF-UPDATED-BATCH-WRITE-CONTROLLED-200-2",
  ...
  console.log(`   rollback_batch_200_2.json written (${BATCH_SIZE} docs)`);
  ...
  const writeSummary = {
    ...
    cumulative: { total_written: 125 + writeCount, total_remaining: 520 - 125 - writeCount },
    rollback_available: "rollback_batch_200_2.json",
  ...
  const recommendation = criticals.length > 0
    ...
    : warns.length > 0
      ...
      : `> [!TIP]\n> Los ${BATCH_SIZE} writes se ejecutaron correctamente. manual.* intacto en todos los casos.\n\n**Recomendacion**: Proceder con el batch restante de **${520 - 125 - writeCount} ISINs**.`;

  const md = `# BDB Morningstar PDF Updated Batch Write Controlled 200-2
...
| **Batch 1 (200-2)** | **${BATCH_SIZE}** | **ESTE BATCH** |
| Restantes | ${520 - 125 - writeCount} | Pendiente |
| Missing (excluidos) | 2 | LU0171281750, LU0171282212 |
| **Total escrito** | **${125 + writeCount}** | |
...
*Batch 1 (100 fondos) â€” Writes: ${writeCount} â€” Total acumulado: ${125 + writeCount}/520*
`;
  ...
  console.log();
  console.log("=".repeat(60));
  console.log("WRITE CONTROLLED 200-2 SUMMARY");
  console.log("=".repeat(60));
  ...
  console.log(`  Cumulative:  ${125 + writeCount}/520 written`);
  console.log(`  Remaining:   ${520 - 125 - writeCount}`);
  console.log(`  Elapsed:     ${Date.now() - startTime.getTime()} ms`);
  console.log("=".repeat(60));
  console.log(criticals.length > 0
    ? "STATUS: CRITICAL FAILURES â€” STOP"
    : "STATUS: WRITE CONTROLLED 200-2 COMPLETE â€” ALL OK");
